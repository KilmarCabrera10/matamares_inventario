...
const cargarDatosIniciales = async () => {
  try {
    ...
    console.log('Saldo Anterior:', saldoAnteriorData)
    console.log('Estadísticas del Día:', statsData)
    console.log('Historial de Cuadres:', historialData)
    ...
    toast.add({
      ...
      description: 'No se pudieron cargar los datos iniciales. Intente recargar la página.',
      color: 'error',
...
const guardarCuadre = async () => {
  try {
    ...
    toast.add({
      title: 'Cuadre guardado exitosamente',
      description: `ID del cuadre: ${respuesta.id}`,
      color: 'primary',
    ...
    toast.add({
      title: 'Error al guardar cuadre',
      description: error instanceof Error ? error.data?.message : 'Error desconocido',
      color: 'error',
          ...
          <UCard class="neutral-card">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div 
                ...
                :class="[
                  diferenciaCuadre >= 0 ? 'bg-green-50 dark:bg-green-900/20' : 'bg-red-50 dark:bg-red-900/20'
                ]"
              >
                <p 
                  class="text-sm font-medium" 
                  :class="[
                    diferenciaCuadre >= 0 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'
                  ]"
                >
                  Diferencia
                </p>
                <p 
                  ...
                  :class="[
                    diferenciaCuadre >= 0 ? 'text-green-900 dark:text-green-100' : 'text-red-900 dark:text-red-100'
                  ]"
                >
            ...
            <div class="grid grid-cols-3 gap-4 items-start">
              ...
              <div class="col-span-1">
                ...
                <div 
                  ...
                  :class="[
                    diferenciaSaldoAnterior >= 0 ? 'bg-green-50 dark:bg-green-900/20 border-green-200 dark:border-green-800' : 'bg-red-50 dark:bg-red-900/20 border-red-200 dark:border-red-800'
                  ]"
                >
                  <div class="flex items-center justify-between w-full">
                    <div class="flex items-center">
                      <Icon 
                        ...
                        class="h-4 w-4 mr-2" 
                        :class="[
                          diferenciaSaldoAnterior >= 0 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'
                        ]"
                      />
                      <span 
                        class="text-sm font-medium" 
                        :class="[
                          diferenciaSaldoAnterior >= 0 ? 'text-green-800 dark:text-green-200' : 'text-red-800 dark:text-red-200'
                        ]"
                      >
                        Dif.
                      </span>
                    ...
                    <span 
                      ...
                      :class="[
                        diferenciaSaldoAnterior >= 0 ? 'text-green-900 dark:text-green-100' : 'text-red-900 dark:text-red-100'
                      ]"
                    >
      ...
      <UCard class="neutral-card h-full flex flex-col">
        ...
        <div class="flex-1 flex flex-col">
          ...
          <div class="space-y-4">
            ...
            <UFormField 
              v-for="billete in billetes" :key="billete.valor" :label="`$${billete.valor}`" class="space-y-1">
              <UInputNumber v-model="conteoEfectivo.billetes[billete.key]" :min="0" :step="1" placeholder="0" size="sm"/>
          ...
          <div class="space-y-4">
            ...
            <div class="grid grid-cols-2 gap-3">
            <UFormField v-for="moneda in monedas" :key="moneda.valor" :label="moneda.label" class="space-y-1">
              <UInputNumber v-model="conteoEfectivo.monedas[moneda.key]" :min="0" :step="1" placeholder="0" size="sm"/>
              <p class="text-xs text-gray-500">
          ...
          <UFormField label="Observaciones" help="Describe cualquier observación sobre el cuadre...">
            <UTextarea v-model="cuadre.observaciones" placeholder="Observaciones del cuadre de caja..." :rows="3" :maxlength="500" resize class="w-full"/>
          </UFormField>
          </div>
          ...
          <div class="mt-6 pt-4 border-t border-gray-200 dark:border-gray-600">
            <div class="flex justify-end space-x-3">
              <UButton color="secondary" variant="solid" class="shadow-sm" @click="limpiarFormulario">
                <Icon name="lucide:refresh-cw" class="h-4 w-4 mr-2" />
                Limpiar
              </UButton>
              <UButton color="neutral" variant="solid" class="shadow-sm bg-blue-700 hover:bg-blue-600 text-white" :loading="guardando" @click="guardarCuadre">
